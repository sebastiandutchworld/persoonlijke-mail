...
class ResUsers(models.Model):
    ...
    @api.model
    def _auth_timeout_deadline_calculate(self, path):
    ...
    def _auth_timeout_check(self):
        ...
        # Calculate deadline
        deadline = self._auth_timeout_deadline_calculate(path)
        # Check if past deadline
        expired = False if deadline != -1 else True
        if (not expired) and (deadline is not False):
